...
public class Player{
    ...
    public void setGameBoard(GameBoard currentBoard) { upeiBoard = currentBoard;  }
    ...
    public void drawCard(int numberOfCards)
    {
        ...
        {
            if (upeiBoard.playerPile.size()!= 0)
            {
                //first element from array list PlayerPile
                if (upeiBoard.playerPile.get(0).equals(Variables.isEpidemic)) {
                    System.out.println("-----EPIDEMIC DRAWN!-----");
                    upeiBoard.resolveEpidemic();//follow the steps for epidemic event
                }
                else
                {
                    //adds a new card to the players hand.
                    hand.add((City) upeiBoard.playerPile.get(0));
                }
                upeiBoard.playerPile.remove(0);//remove the card from PlayerDeck
    ...
    public void discardCard(String cardName)
    {
        ...
        upeiBoard.addPlayerCardDiscard(hand.get(toDiscard));//remove from playerDeck to put in PlayerDiscardDeck
    ...
    public boolean treatDisease (City location, String colour)
    {
        ...
        if (playerRole.equals("MEDIC")) {
            if(tmp.isaLegalMove() && location == playerPiece.getLocation())
            {
                ...
                for (int i=0;i<location.getCubeColour(colour);i++) {
                    ...
                    upeiBoard.addCube(colour);//add in pool of cube
                }
                decreasePlayerAction();
                return true;
            }
    ...
    public boolean discoverCure(City location, String colour)
    {
        ...
        if (playerRole.equals("SCIENTIST")) {
            if (tmp.isaLegalMove(1))
            {
                ...
                discardCureCards(colour,upeiBoard.getNeededForCure()-1);
                upeiBoard.cureDisease(colour);
                decreasePlayerAction();
                return true;
            }
        }
        else
        {
            ...
            {
                System.out.println("Its possible to discover a cure");
                discardCureCards(colour,upeiBoard.getNeededForCure());
                upeiBoard.cureDisease(colour);
                decreasePlayerAction();
                return true;
            }
    ...
    public boolean checkTryCure()
    {
        if (discoverCure(playerPiece.getLocation(),tryCureCardColour())) {
            ...
            System.out.println("They need to go to a research station.");
    ...
    public boolean checkCureWorthIt()
    {
        ...
        if (toCure != null)
        {
            for (int i = 0 ; i < upeiBoard.diseases.size() ; i ++){

                Disease disease = upeiBoard.diseases.get(i);
    ...
    {
        for (int i = 0; i < upeiBoard.getNumberColours(); i ++)
        {
            if (getCountXCards(possibleColour[i]) >= upeiBoard.getNeededForCure())
            {
    ...
    {
        getDistances(upeiBoard.getResearchLocations());
    ...
    {
        upeiBoard.resetDistances();
        setDestination(destinations);
        int distance = 1;
        while (playerPiece.getLocation().getDistance() == 9999999)
        {
            // System.out.println("Looking for places distance of " + distance);
            for (int i = 0 ; i < upeiBoard.cities.size() ; i ++)
            {

                for (int x = 0 ; x < 6 ; x ++)
                {
                    if (upeiBoard.cities.get(i).getDistance() == (distance-1)
                            && upeiBoard.cities.get(i).getNeighbors().get(x).getDistance() > distance)
                    {
                        upeiBoard.cities.get(i).getNeighbors().get(x).setDistance(distance);
                    }
                }
    ...
    public void charterRandom(){
        ...
        int n = rand.nextInt(upeiBoard.cities.size());
        charterFlight(playerPiece.getLocation(),upeiBoard.cities.get(n));
    }
    ...
    {
        ArrayList<City> CitiesWith3Cubes =  upeiBoard.get3CubeCities();
    ...
    {
        ArrayList<City> CitiesWith2Cubes = upeiBoard.get2CubeCities();
    ...
    {
        ArrayList<City> CitiesWith1Cubes = upeiBoard.get1CubeCities();
    ...
    public boolean enoughCardsForCure(){
        ...
        return n == redcards || n == greencards || n == bluecards || n == yellowcards;
    }
